    ...
    int longestConsecutive(vector<int>& nums) {
        ...
        for(int num : nums){
            if(!hashMap.contains(num - 1) && !hashMap.contains(num + 1))
                ...
                hashMap[num] = {num, hashMap.contains(num) ? max(hashMap[num].second, hashMap[num + 1].second) : hashMap[num + 1].second};
                hashMap[hashMap[num + 1].second] = {num, hashMap[num].second};
            }
            else if (hashMap.contains(num - 1) && !hashMap.contains(num + 1)){
                hashMap[num] = {hashMap.contains(num) ? min(hashMap[num - 1].first, hashMap[num].first) : hashMap[num - 1].first, num};
                hashMap[hashMap[num - 1].first] = {hashMap[num].first, num};
            }
            else{
                hashMap[num] = {hashMap.contains(num) ? min(hashMap[num - 1].first, hashMap[num].first) : hashMap[num - 1].first, hashMap.contains(num) ? max(hashMap[num].second, hashMap[num + 1].second) : hashMap[num + 1].second}; 
